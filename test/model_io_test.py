...
metadata = ModelIO.extract_metadata(model)
ModelIO.save_model(model, backend, file_name="test_model", meta=metadata)

reloaded_model, loaded_metadata = ModelIO.load_model("models/test_model.pkl")

print(loaded_metadata if loaded_metadata else "Metadata is not provided")

for p1, p2 in zip(model.parameters(), reloaded_model.parameters()):
    print(
        f"{"Serialized data matches after deserialization" if (p1.data == p2.data).all() else "Deserializaion Failure"}"
    )
